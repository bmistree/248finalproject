...
NUM_TREES_PER_ROW = 3;
NUM_TREES_PER_COL = 4;
//NUM_INITIAL_TREES = NUM_TREES_PER_ROW*NUM_TREES_PER_COL* 5;
NUM_INITIAL_TREES = 220;
// NUM_INITIAL_TREES = NUM_TREES_PER_ROW*NUM_TREES_PER_COL* 2;
...
{
    this.total_num_trees_added += num_trees_to_add;
    var tree_pos = { x:0,y:.7,z:0 };
    for (var index = 0; index < num_trees_to_add; ++index)
    {
        ...
        // need to block light when additively blending.
        var block_tree = new THREE.Mesh(new_tree.geometry.clone(), black_material);
        block_tree.position = new_tree.position;
        block_tree.rotation = new_tree.rotation;
        block_tree.scale = new_tree.scale;
        glow_scene.add(block_tree);

        
        // glow_scene.add(new_tree);
...
{
    this.add_trees(Math.max(num_trees_to_get - this.available_trees.length,0));

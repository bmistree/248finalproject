...
{
    this.tree_pool = new TreePool(150);
...
TreePool.prototype.add_trees = function (num_trees_to_add)
{
    ...
    for (var index = 0; index < num_trees_to_add; ++index)
        this.available_trees.push(draw_tree(tree_pos));
};
...
{
    this.available_trees.concat(trees);
};
...
TreeTile.prototype.hide = function()
{
    ...
    console.log('Hiding for corner: ' + this.southwest_x_coord + '   ' + this.southwest_z_coord);
    
    this.painted = false;
    
    // remove each tree from the scene
    for (var tree_index in this.trees)
        scene.remove(this.trees[tree_index]);
...
TreeTile.prototype.paint = function()
{
    ...
    console.log('Painting for corner: ' + this.southwest_x_coord + '   ' + this.southwest_z_coord);
    
    this.painted = true;
    this.populate_trees();
    for (var tree_index in this.trees)
        scene.add(this.trees[tree_index]);
};

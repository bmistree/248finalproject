...
DrawingConsts.NUM_TREES = 20;
//DrawingConsts.NUM_TREES = 400;
...
function load_scene_map(scene_json_url)
{
    ...
    var x_tree_spacing = 5;
    var z_tree_spacing = -5;
    for (var x_tree_index = 0; x_tree_index < Math.sqrt(DrawingConsts.NUM_TREES);
         ++x_tree_index)
    {
        for (var z_tree_index = 0; z_tree_index < Math.sqrt(DrawingConsts.NUM_TREES);
             ++z_tree_index)
        {
            tree_obj.pos.x = x_pos_base + x_tree_index*x_tree_spacing;
            tree_obj.pos.z = z_pos_base + z_tree_index*z_tree_spacing;
...
//    ++seed;
    ...
    DrawingConsts.TREE_MESH_DUPLICATION_FACTOR = 1;
    var num_original_tree_verts = tree.verts.length;
    for (var dup_index = 0; dup_index < DrawingConsts.TREE_MESH_DUPLICATION_FACTOR; ++dup_index)
    {
    ...
    console.log(num_original_tree_verts);
    console.log(tree.faces.length);
    for (dup_index = 0; dup_index < DrawingConsts.TREE_MESH_DUPLICATION_FACTOR; ++dup_index)
    {

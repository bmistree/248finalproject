...
DrawingConsts.GROUND_SOUTHWEST_Y = -4;
DrawingConsts.GROUND_SOUTHWEST_Z = 0;
DrawingConsts.GROUND_SIDE_LEN = 320;

DrawingConsts.STARTING_EYE_X = 100;
DrawingConsts.STARTING_EYE_Y = 3.5;
DrawingConsts.STARTING_EYE_Z = 100;

DrawingConsts.FRAME_PERIOD_MS = 10;
DrawingConsts.DOOR_GLOW_FRAME_PERIOD_MS = 60;
...
DrawingConsts.SERVER_WEBSOCKET_ADDRESS = 'ws://bcoli.stanford.edu:18080/ws';
...
// unique id assigned by server.. populated in first message of send_msg
var my_id;
...
var plant_manager,fire_manager,opponent_manager;
var sound_manager;
...
function drawing_init()
{
    ...
    // radar
    radar = new Radar(DrawingConsts.DRAWING_DIV);
    ...
    // keypresses for in-game actions
    //$('#' + DrawingConsts.DRAWING_DIV).keypress(handle_key_press);
    $('body').keypress(handle_key_press);
...
function setup_game_managers()
{
    ...
    fire_manager = new FireManager();
    opponent_manager = new OpponentManager();
}
...
function animate_doors()
{
    ...
    setTimeout(
        ...
        DrawingConsts.DOOR_GLOW_FRAME_PERIOD_MS);
}    
...
function setup_two_phase_rendering()
{
    ...
    hblur.uniforms[ "h" ].value = 5 / DrawingConsts.DIV_WIDTH;
    vblur.uniforms[ "v" ].value = 5 / DrawingConsts.DIV_HEIGHT;
    glow_composer.addPass( hblur );
    glow_composer.addPass( vblur );


    // var effect_bloom = new THREE.BloomPass( .15 );
    // glow_composer.addPass(effect_bloom);
    ...
    //var effect_bloom = new THREE.BloomPass( 1.25 );
    //var effect_bloom = new THREE.BloomPass( .1 );
    //final_composer.addPass(effect_bloom);
    // effect_bloom.renderToScreen = true;

    // // var hblur_2 = new THREE.ShaderPass( THREE.HorizontalBlurShader );
    // var vblur_2 = new THREE.ShaderPass( THREE.VerticalBlurShader );
    // // // // hblur_2.uniforms[ "h" ].value = 1 / DrawingConsts.DIV_WIDTH;
    // // // //vblur_2.uniforms[ "v" ].value = 1 / DrawingConsts.DIV_HEIGHT;
    // vblur_2.uniforms[ "v" ].value = 0;
    // // // final_composer.addPass(hblur_2);
    // final_composer.addPass(vblur_2);
    // vblur_2.renderToScreen = true;
}

...
RADAR_WINNING_OPPONENT_COLOR = '#000000';
RADAR_WINNING_OPPONENT_RADIUS = RADAR_CAMERA_RADIUS;
...
Radar.prototype.update = function()
{
    if (plant_manager !== undefined)
    {
        ...
        for (var other_plant_index = 0; other_plant_index < plant_manager.other_plants.length;
             ++other_plant_index)
        {
            var plant = plant_manager.other_plants[other_plant_index];
            var plant_pos = this.convert_position(plant.base_point);
            this.draw_plant(plant_pos,RADAR_OTHER_PLANT_COLOR);
        }

        if (gold_manager !== undefined)
        {
            // also display a circle for the position of the player
            // with the highest score

            // if it equaled null, meant that local player currently
            // had most points
            var opp_winning_id = gold_manager.scoreboard.get_winning_id();
            if (opp_winning_id !== null)
            {
                if (opp_winning_id in opponent_manager.opponents)
                {
                    var winning_opponent = opponent_manager.opponents[opp_winning_id];

                    var winning_opponent_position = this.convert_position(
                        winning_opponent.mesh.position);
                    
                    this.context.fillStyle = RADAR_WINNING_OPPONENT_COLOR;
                    this.context.beginPath();
                    this.context.arc(
                        winning_opponent_position.x,winning_opponent_position.y,     
                        RADAR_WINNING_OPPONENT_RADIUS, 0, Math.PI*2, true); 
                    this.context.closePath();
                    this.context.fill();
                }
            }

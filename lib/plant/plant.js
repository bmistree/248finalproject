    ...
    base_point,fidelity,growth_fidelity, base_plant)
{
    ...
    this.amount_to_scale_by_each_grow = 1/ NUM_GROWS_BEFORE_FULL;
    this.num_grows_called = 0;
    
    // the branch that we are going to grow
    this.shoot = new THREE.Mesh(
        new THREE.CylinderGeometry(
            ...
            .1,
            // height
            branch_len/NUM_GROWS_BEFORE_FULL,
            // segment radius
            10,
            // segments height
            10,
    ...
    var angle_of_rotation = Math.acos(this.growth_direction.y);
    if (angle_of_rotation != 0)
    {
        ...
        var rot_matrix = new THREE.Matrix4().makeRotationAxis( axis_of_rotation, angle_of_rotation );
        this.shoot.applyMatrix(rot_matrix);
    }
    
    // set the base of the cylinder
    this.shoot.position.x = this.base_point.x;
    this.shoot.position.y = this.base_point.y;
    this.shoot.position.z = this.base_point.z;
...
Plant.prototype.grow = function(growth_particles)
{
    ...
    if (this.num_grows_called == 0)
        this.base_plant.add(this.shoot);
    
    ++ this.num_grows_called;
    this.shoot.scale.set(1, this.num_grows_called,1);
    ...
    var midpoint_growth_point = new THREE.Vector3();
    midpoint_growth_point.x = this.base_point.x + fraction_grown * (this.distant_point.x - this.base_point.x)/2;
    midpoint_growth_point.y = this.base_point.y + fraction_grown * (this.distant_point.y - this.base_point.y)/2;
    midpoint_growth_point.z = this.base_point.z + fraction_grown * (this.distant_point.z - this.base_point.z)/2;
    
    this.shoot.position.x = midpoint_growth_point.x;
    this.shoot.position.y = midpoint_growth_point.y;
    this.shoot.position.z = midpoint_growth_point.z;

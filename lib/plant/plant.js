...
function Plant(
    ...
    base_point,fidelity,growth_fidelity, base_plant,plant_joint_material)
{
    direction.normalize();
    
    this.plant_joint_material = plant_joint_material;
    this.root = false;
    this.depth = depth;
...
Plant.prototype.grow = function(growth_particles)
{
    ...
    if (this.num_grows_called >= NUM_GROWS_BEFORE_FULL)
    {
        ...
        // means that we have finished growing the entire plant
        if (this.root)
        {
            this.particles.stop();
            // release particles... no longer need them.
            this.particles = undefined;
        }

        // true means that we are done growing.
        return true;
    }
    
    var base_width = depth_to_width_map[this.depth];
    var end_width = depth_to_width_map[this.depth-1];
    
    // update position of particles
    var new_growth_point = new THREE.Vector3();
    var fraction_grown = this.num_grows_called / NUM_GROWS_BEFORE_FULL;
    var next_fraction_grown = (this.num_grows_called + 1)/ NUM_GROWS_BEFORE_FULL;
    ...
    var joint_base_width = base_width*(1-fraction_grown) + end_width*(fraction_grown);
    var joint_end_width = base_width*(1-next_fraction_grown) + end_width*(next_fraction_grown);
    
    // create a new joint to append on the plant
    var cylinder_height = (this.branch_len/NUM_GROWS_BEFORE_FULL) + .1;
    var joint_part = new THREE.Mesh(
        new THREE.CylinderGeometry(
            ...
            // open ended
            true),
        this.plant_joint_material);

    // rotate the cylinder across its base
    joint_part.geometry.applyMatrix(
        new THREE.Matrix4().makeTranslation( 0, cylinder_height/2, 0 ) );
    ...
    base_point,fidelity,growth_fidelity, base_plant)
{
    ...
    if (depth != 0)
    {
        ...
        var new_branch = primary_branch_deviation(
            direction,depth,branch_range,branch_len_range,gnarl_factor,
            this.distant_point,
            fidelity,growth_fidelity,this.base_plant,this.plant_joint_material);
...
function primary_branch_deviation(
    ...
    base_point,prev_fidelity, prev_growth_fidelity,base_plant,plant_joint_material)
{
    ...
    return new Plant(
        ...
        fidelity,growth_fidelity,base_plant,plant_joint_material);
}

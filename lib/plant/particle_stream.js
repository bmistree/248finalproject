...
ParticleStream.prototype.produce_point_paths = function()
{
    ...
    control_points.push(
        new THREE.Vector3(this.dest_point.x,this.dest_point.y,this.dest_point.z));
    
    this.point_paths =
        new THREE.SplineCurve3(control_points).getPoints(STREAM_FIDELITY);

    var final_point = this.point_paths[this.point_paths.length -1];
    var expected_final_point = this.dest_point;
    var end_control = control_points[control_points.length -1];
    console.log('Final ' + final_point.x + ' ' + final_point.y + ' ' + final_point.z);
    console.log('Expected ' + expected_final_point.x + ' ' + expected_final_point.y + ' ' + expected_final_point.z);
    console.log('Control ' + end_control.x + ' ' + end_control.y + ' ' + end_control.z);
...
/**
 * If any point reaches destination, then return number removed from
 * the path. Otherwise, move points
 * further along path.
 */
ParticleStream.prototype.update = function()
{
    ...
    return indices_to_remove.length;
};
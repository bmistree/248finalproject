...
function GoldManager ()
{
    ...
    $('#' + DrawingConsts.DRAWING_DIV).append(
        '<div id="' + GOLD_DISPLAY_DIV + '" ' +
            ...
            'position: relative; top: -620px; left: 950px;"' +
            '></div>');
    this.update_div();
}

GoldManager.prototype.handle_player_data_message = function (player_data_message)
{
    if (player_data_message.Me)
    {
        this.holding_gold = player_data_message.GoldAmt;
        this.points = player_data_message.Points;
    }
    else
        opponent_manager.handle_player_data_message(player_data_message);
};
...
GoldManager.prototype.update_div = function ()
{
    ...
    var to_display_text =
        '<table> ' +
        '<tr><td>POINTS</td><td></td>' +
        '</tr><tr>' +
        '<td>Score: </td><td>' + this.points.toFixed(2) + '</td>' + 
        '</tr><tr>' +
...
GoldManager.prototype.handle_gold_message = function (gold_message)
{
    ...
    if (gold_message.DeletedSubmessages !== null)
    {
             ...
             ++deleted_index)
        {
            ...
            delete this.gold_map[deleted_stash_info.StashId];
        }
    }
